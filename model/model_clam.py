...
class CLAM_MB(CLAM_SB):
    def __init__(self, gate = True, cluster_id=None, path_input_dim=1024, size_arg = "small", dropout = False, k_sample=8, n_classes=2,
        instance_loss_fn=nn.CrossEntropyLoss(), subtyping=False):
    ...
    def forward(self, h, label=None, cluster_id=None, instance_eval=False, return_features=False, attention_only=False):
        device = h.device
        A, h = self.attention_net(h)  # NxK        
        A = torch.transpose(A, 1, 0)  # KxN
        if attention_only:
