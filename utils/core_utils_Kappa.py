...
class CCE(nn.Module):
    def __init__(self, balancing_factor=1):
    ...
    def forward(self, yHat, y):
        ...
        output = Px * Px_log * y_zerohot.to(y.device)
...
def train(datasets, cur, args):
    ...
    if args.bag_loss == 'svm':
        ...
        loss_fn = FocalKappaLoss(n_classes=2)
        # loss_fn = CCE()
    print('Done!')
    ...
    if args.model_type in ['clam_sb', 'clam_mb']:
        ...
        if args.model_type == 'hipt_n':
            ...
            model = HIPT_LGP_FC(**model_dict, freeze_4k=args.freeze_4k, pretrain_4k=args.pretrain_4k, freeze_WSI=args.freeze_WSI, pretrain_WSI='None')
        elif args.model_type == 'hipt_gp':
            model = HIPT_GP_FC(**model_dict, freeze_WSI=args.freeze_WSI, pretrain_WSI='None')
    ...
    for epoch in range(args.max_epochs):
        if args.model_type in ['clam_sb', 'clam_mb'] and not args.no_inst_cluster:
            train_loop_clam(epoch, model, train_loader, optimizer, args.n_classes, args.bag_weight, writer, loss_fn, dropinput=0)
